...
THERE IS SO MUCH I DON'T UNDERSTAND BETWEEN V.4 AND V.5--- RAHHHHHHHHHHHH!!!!!
...
class WhackAMole:
	...
	def __init__(self):
		...
		self.mole_cover_photo = PhotoImage(file="mole_cover.gif") #<---wtf is this?
		self.label_timers = {}
		...
		self.set_callbacks()
		self.game_is_running = False #<--- What is this?
	
	def create_frames(self):
		mole_frame = tk.Frame(self.window)
		mole_frame.grid(row=0, column=0) #<--- Why Zero now?
		
		status_frame= tk.Frame(self.window, bg=WhackAMole.STATUS_BACKGROUND)
		status_frame.grid(row=0, column=1, sticky= tk.E + tk.W + tk.S + tk.N, ipadx=6) #<---RSCH what ipadx parameter is
        ...
        # Source of mole image: https://play.google.com/store/apps/details?id=genergame.molehammer (Huh?- this is not a link to a static image, but a playable whackamole game??)
	 	...
	 	for r in range(WhackAMole.NUM_MOLES_ACROSS):
	 		...
	 		for c in range(WhackAMole.NUM_MOLES_ACROSS):
	 			...
	 			mole_label.grid(row=r, column=c, sticky=tk.E + tk.W + tk.N + tk.S) #<---Why no padx or pady parameters?
	...
	def mole_hit(self, event):#<---Why extra parameter of event added?
		
		if self.game_is_running:
			hit_label = event.widget#<---Where is this .widget attribute coming from/defined?
		...
		if self.start_button['text'] == 'Start':
			...
			for r in range(WhackAMole.NUM_MOLES_ACROSS):
				for c in range(WhackAMole.NUM_MOLES_ACROSS):
					...
					the_label['image'] = self.mole_cover_photo#<---WTF IS THE COVER PHOTO???
					time_down = randint(WhackAMole.MIN_TIME_DOWN, WhackAMole.MAX_TIME_DOWN)
					timer_object = the_label.after(time_down, self.pop_up_mole, the_label)#<---RSCH WTF this is doing
	...
	def put_down_mole(self, the_label, timer_expired):
		if self.game_is_running:
			if timer_expired:
				#The mole is going down before it was clicked on, so update the counter
				self.miss_counter['text'] = str(int(self.miss_counter['text']) + 1)
			...
			the_label['image'] = self.mole_cover_photo#<---FUCKING COVER PHOTO WHAT IS IT??
